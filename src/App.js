                ...
                <form onSubmit={handleAuthAction}>
                    {!isLogin && (
                        <div className="mb-4">
                            ...
                            <input type="text" id="username" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full bg-black/20 p-3 rounded-md border border-white/20 focus:outline-none focus:ring-2 focus:ring-primary" required />
                        </div>
                    )}
                    <div className="mb-4">
                        <label className="block text-gray-300 mb-2" htmlFor="email">Email</label>
                        <input type="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-black/20 p-3 rounded-md border border-white/20 focus:outline-none focus:ring-2 focus:ring-primary" required />
                    </div>
                    <div className="mb-6">
                        <label className="block text-gray-300 mb-2" htmlFor="password">Password</label>
                        <input type="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-black/20 p-3 rounded-md border border-white/20 focus:outline-none focus:ring-2 focus:ring-primary" required />
                    </div>
                    {error && <p className="text-danger text-center mb-4">{error}</p>}
                    <button type="submit" className="w-full bg-primary hover:opacity-90 font-bold py-3 rounded-md transition duration-300 disabled:opacity-50" disabled={loading}>
        ...
        <div className="w-64 glass-card h-screen flex-shrink-0 flex flex-col p-4">
            ...
            <div className="border-t border-white/20 pt-4">
                 <div className="flex items-center p-3 rounded-lg text-white">
                    <ProfileIcon />
                    <span className="ml-3">{user.username || 'Player'}</span>
                </div>
...
// --- Page Content Components (Placeholders) ---
const PageContent = ({ title }) => (
    <div className="p-8 text-white">
        <h1 className="text-4xl font-bold">{title}</h1>
        <p className="mt-4 text-gray-300">Content for the {title} page goes here.</p>
    </div>
);

const HomePage = () => <PageContent title="Home" />;
const MyCompetitions = () => <PageContent title="My Competitions" />;
const Explore = () => <PageContent title="Explore" />;
const Admin = () => <PageContent title="Admin Dashboard" />;
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'explore':
                return <Explore />;
            case 'admin':
                return user?.role === 'admin' ? <Admin /> : <HomePage />;
            default:
                return <HomePage />;
        }
    };
